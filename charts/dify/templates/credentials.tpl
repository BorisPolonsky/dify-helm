...
{{- $celeryBrokerDB := .celeryBrokerDB | default 1 }}
{{- $sentinelUrls := list }}
{{- range $sentinel := .sentinel.sentinels }}
{{- $sentinelUrls = append $sentinelUrls (printf "sentinel://:%s@%s/%v" $redisPassword $sentinel $celeryBrokerDB) }}
{{- end }}
...
CELERY_SENTINEL_PASSWORD: {{ .sentinel.password | b64enc | quote }}
    {{- else }}
      ...
      {{- if .useSSL }}
        {{- $scheme = "rediss" }}
      {{- end }}
CELERY_BROKER_URL: {{ printf "%s://%s:%s@%s:%v/%v" $scheme .username .password .host .port .celeryBrokerDB | b64enc | quote }}
    {{- end }}
  {{- end }}
{{- else if .Values.redis.enabled }}
